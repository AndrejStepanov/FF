	...
	<v-card height="100%" >
		<v-card-title v-if="tableTitle!='' || searchNeed" ref="title" class='noPadding'>
			...
			<template v-if="searchNeed">
				<v-spacer/>
				<v-text-field v-model="search"	append-icon="search" :label="$vuetify.lang.t('$vuetify.system.simple.labels.searchInFields')"	single-line	hide-details clearable />				
			</template>
			<template v-if="fiterButtonhNeed">
				<v-spacer/>
				<v-btn  class="mr-2" color="accent" @click="$emit('fiterButtonClick')"  > {{$vuetify.lang.t('$vuetify.system.simple.labels.filter')}} <v-icon right>filter_list</v-icon>   </v-btn>			
			</template>
		</v-card-title>
...
<script>
	...
	export default {
		...
		computed: {
			...
			tabHeads(){
				let  vm = this,
					tmp = [vm.withRowNum?{value:'_id', text:vm.$vuetify.lang.t('$vuetify.system.simple.labels.numInOrder'), type:'int', clsssCell:'width-one-percent'}:{}]
				...
				return  tmp.concat( vm.headers).map((head,i) => {
					...
					return {...head, visible:( head.visible===false?false:true ), sortable:( head.sortable===true?true:false ), 
						_isNumeric:( _isNumeric || _isInt), _isDate, mask: vm.$h.nvl(head.mask, ( _isNumeric?'0,0.000': _isInt ?'0,0':undefined ) ),
						clsssCell:vm.$h.nvl(head.clsssCell,'')+' '+(_isNumeric||_isDate?' text-nobr ':'')+' '+(_isNumeric?' text-right ':''),
					}
				})
			},
			tabHeadsVisible(){

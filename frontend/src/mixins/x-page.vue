<script>
	...
	export default {
		...
		async created (){
			let vm=this
			vm.componentName= vm.$h.camelize(vm.$h.nvl(this.$route.meta.compName, vm.componentName, vm.$vnode.tag.replace( /(vue-component-\d+-)(\w+)/ ,'$2') ) )
			vm.componentName= vm.componentName[0].toUpperCase()+vm.componentName.slice(1)
			for (let group in vm.inputsConfig)
				await vm.paramConfigInit({ form: group , params:vm.inputsConfig[group], type:'inputs'	})
			for (let group in vm.filtersConfig)
				await vm.paramConfigInit({ form: group , params:vm.filtersConfig[group], type:'filters'	})
			await vm.initButtons(vm.buttonsConfig)
			await vm.initDialogs(vm.dialogsConfig)
			await vm.initTables(vm.tablesConfig)
			vm.$root.$on(vm.componentName+'_contentRefresh', async ( data )=>{
				await vm.contentRefresh( data )
			})
		},

export default {
	...
	getters: { // computed properties
		...
		getValByCode: (state,getters) =>(form,code)=> {
			...
			return nvlo(data,'')==''?null: data[ getters.getValFieldByData(data) ]
		},
		...
		getTodo: (state,getters) =>(form)=> {
			return getters.getGroup(form)
		},
		getTodoChecked: (state,getters) =>(form)=> {
	...
	actions:{	
		async doInit({dispatch,commit,getters,state},{form,params}){
			...
			commit("allParamsSet",{ form, params: params.reduce((res, row) => ({ 
				...res, [row.code]:{
					sign:'=', need_reset:true, checked:true, defaultInit:false, value: '', valueView:'',value_arr: ['',''], valueArrView:['',''], table_values:[], table_values_init:false,  
		...
		async doSetValOutside({commit,getters,state},{form,code, value, data}){
			let param =getters.getByCode(form,code),valField='value', type = typeOfObject(value), promise={}
			data=data||{}
			if(nvlo(param,'')=='')
				return
			...
			commit("paramSetting",{ form,code, data:{...data, need_reset:true, [ valField ]:value } })		
		},

...
//<sub> {{$vuetify.lang.t('$vuetify.texts.simple.labels.hourBrevis')}} </sub>
//<sub>{{$vuetify.lang.t('$vuetify.texts.simple.labels.minuteBrevis')}}</sub>
//<sub>{{$vuetify.lang.t('$vuetify.texts.simple.labels.secondBrevis')}}</sub>
	export default {
		name:'c-time-picker',
		data: () => ({
			changing: false,
		...
		computed: {
			...
			hour: {
				...
				get:function() 		{return nvl(this.value.split(':')[0],'00')																							},
			},
			minute: {
				set:function (val)	{this.$emit('input', this.hour+':'+val+':'+this.second)																				},
				get:function() 		{return nvl(this.value.split(':')[1],'00')																							},
			},
			second: {
				set:function (val)	{this.$emit('input', this.hour+':'+this.minute+':'+val)																				},
				get:function() 		{return nvl(this.value.split(':')[2],'00')																							},
			},
			minHour () {

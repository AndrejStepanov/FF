export default {
	...
	props: {
		...
		parentW: {type: Number,default: 0, validator:  (val)=> {return val >= 0}},
		parentH: {type: Number,default: 0,validator:  (val) =>{return val >= 0}},
		w: {type: Number,default: 100,validator:  (val) =>{return val > 0}},
		h: {type: Number,default: 100,validator:  (val) =>{return val > 0}},
		minw: {type: Number,default: 50,validator:  (val) =>{return val > 0}},
		minh: {type: Number,default: 50,validator:  (val) =>{return val > 0}},
		x: {type: Number,default: 0,validator:  (val) =>{return typeof val === 'number'}},
		y: {type: Number,default: 0,validator:  (val) =>{return typeof val === 'number'}},
	...
	methods: {
		...
		stickDown: function (stick, ev) {
			...
			switch (this.currentStick[0]) {
				case 'b':this.stickAxis = 'y';break;
				case 't':this.stickAxis = 'y';break;
			}
			switch (this.currentStick[1]) {
				case 'r':this.stickAxis = this.stickAxis === 'y' ? 'xy' : 'x';break;
				case 'l':this.stickAxis = this.stickAxis === 'y' ? 'xy' : 'x';break;
			}
		...
		stickMove(ev) {
			...
			switch (this.currentStick[0]) {
				case 'b':this.rawBottom = stickStartPos.bottom + delta.y;break;
				case 't':this.rawTop = stickStartPos.top - delta.y;break;
			}
			switch (this.currentStick[1]) {
				case 'r':this.rawRight = stickStartPos.right + delta.x;break;
				case 'l':this.rawLeft = stickStartPos.left - delta.x;break;
			}
